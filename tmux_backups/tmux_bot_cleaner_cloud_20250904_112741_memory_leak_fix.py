...
def create_tmux_log_table():
    """Create tmux_log table if it doesn't exist"""
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if not existing_columns:
                ...
                cursor.execute("""
                    CREATE TABLE tmux_log (
                        id SERIAL PRIMARY KEY,
                        file_name VARCHAR(255) NOT NULL,
                        timestamp TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
                        alert BOOLEAN DEFAULT FALSE,
                        log_count VARCHAR(50) DEFAULT '01/01-01 | 1'
                    )
                """)
                log_event("✅ tmux_log table created successfully")
            else:
                # Table exists, add missing columns if needed
                if 'file_name' not in existing_columns:
                    cursor.execute("ALTER TABLE tmux_log ADD COLUMN file_name VARCHAR(255)")
                    log_event("✅ Added file_name column to tmux_log table")
                
                if 'alert' not in existing_columns:
                    cursor.execute("ALTER TABLE tmux_log ADD COLUMN alert BOOLEAN DEFAULT FALSE")
                    log_event("✅ Added alert column to tmux_log table")
                
                if 'log_count' not in existing_columns:
                    cursor.execute("ALTER TABLE tmux_log ADD COLUMN log_count VARCHAR(50) DEFAULT '01/01-01 | 1'")
                    log_event("✅ Added log_count column to tmux_log table")
                
                log_event("✅ tmux_log table structure verified/updated")
            
            conn.commit()
            return True
    except Exception as e:
        log_event(f"⚠️ Failed to create/update tmux_log table: {e}")
        return False
    finally:
        conn.close()
...
def update_tmux_cleaner_status():
    ...
    try:
        with conn.cursor() as cursor:
            # Check if tmux_cleaner entry exists
            cursor.execute("SELECT id FROM tmux_log WHERE file_name = 'tmux_cleaner'")
            if cursor.fetchone():
                # Update existing entry
                cursor.execute("""
                    UPDATE tmux_log 
                    SET timestamp = NOW(), alert = FALSE 
                    WHERE file_name = 'tmux_cleaner'
                """)
            else:
                # Insert new entry
                cursor.execute("""
                    INSERT INTO tmux_log (file_name, timestamp, alert, log_count) 
...
def get_bot_timestamp_from_db(script_path):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT timestamp FROM tmux_log WHERE file_name = %s ORDER BY timestamp DESC LIMIT 1", (script_path,))
...
def get_log_count():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT log_count FROM tmux_log ORDER BY timestamp DESC LIMIT 1")
            result = cursor.fetchone()
...
def should_restart_bot_ram_only(script_path):
    ...
    # Only restart if memory usage is problematic
    if total_memory == 0:
        ...
        log_event(f"🚨 Bot {script_path} memory leak detected ({memory_mb:.2f} MB) - will restart")
        return True
...
def should_restart_bot_timestamp(script_path, alarm_minutes, restart_minutes):
    ...
    if db_timestamp:
        ...
        if process_start_time < db_timestamp_unix:
            ...
            log_event(f"📊 Using process start time for {script_path} (file started time)")
        else:
            # Process started AFTER DB timestamp → Use DATABASE time
            reference_time = db_timestamp_unix
            timestamp_source = "database_timestamp"
            log_event(f"📊 Using database timestamp for {script_path}")
    else:
        # No database timestamp, use process start time
        reference_time = process_start_time
        timestamp_source = "process_start_time"
        log_event(f"📊 No DB timestamp for {script_path}, using process start time")
    ...
    if time_since_reference >= alarm_seconds:
        ...
        if time_since_reference >= restart_seconds:
            log_event(f"🚨 Bot {script_path} exceeded restart threshold ({restart_minutes}m) - Restarting")
            return True
        else:
            remaining_restart = restart_seconds - time_since_reference
            log_event(f"⚠️ Bot {script_path} exceeded alarm threshold ({alarm_minutes}m) - Restart in {remaining_restart/60:.1f}m")
            return False
    else:
        remaining_alarm = alarm_seconds - time_since_reference
        log_event(f"📊 Bot {script_path} healthy - Next alarm in {remaining_alarm/60:.1f}m (using {timestamp_source})")
        return False
